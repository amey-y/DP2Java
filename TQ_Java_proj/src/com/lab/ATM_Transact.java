...
public class ATM_Transact {
	...
	public static void main(String[] args) {
		...
		while (i < 3) // to check PIN match and give max 3 attempts.
		{
			...
			int pin = sc.nextInt();
			if (pin == secureCode) {
				isPIN = true; // if entered PIN matches successfully then isPIN = true & it breaks the while
								// loop.
				break;
			} else {
				i++; // it increments number of attempts.
				System.out.println("You Entered the wrong PIN " + i + " time(s)\nYou have " + (3 - i)
						+ " more attempt(s) remaining.");
			}
		}
		...
		if (isPIN) {
			do {
				...
				System.out.println("Do you want to Continue? (Y/N): "); // Giving choice to continue and storing that
																		// into 'ch'
				ch = sc.next().charAt(0);
